...
test('Import file dalam format JSON melalui klik pada area drag&drop', async ({ page }) => {
  ...
  const before = await target.screenshot({ path: 'tests/screenshots/TC-30/before.png' });
  ...
  const after = await target.screenshot({ path: 'tests/screenshots/TC-30/after.png' });
