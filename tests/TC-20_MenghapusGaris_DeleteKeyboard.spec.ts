...
test('Menghapus garis di kanvas dengan menggunakan tombol delete pada keyboard', async ({ page }) => {
  ...
  const before = await target.screenshot({path: 'tests/screenshots/TC-20/before.png'});
  ...
  const after = await target.screenshot({path: 'tests/screenshots/TC-20/after.png'});
