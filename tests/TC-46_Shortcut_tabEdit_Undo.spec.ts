...
test('Menggunakan shortcut "Undo" yang valid melalui tab Edit', async ({ page }) => {
  ...
  const before = await target.screenshot({path: 'tests/screenshots/TC-46/before.png'});

  // Valid shortcut undo melalui tab Edit
  await page.getByRole('button', { name: 'EDIT' }).click();
  await page.getByRole('button', { name: 'Undo Ctrl+Z' }).click();

  // Verifikasi bahwa shortcut berhasil sesuai dengan opsi edit yang dipilih
  const after = await target.screenshot({path: 'tests/screenshots/TC-46/after.png'});
