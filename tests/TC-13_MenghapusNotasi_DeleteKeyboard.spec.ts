...
test('Menghapus notasi di kanvas dengan menggunakan tombol delete pada keyboard', async ({ page }) => {
  ...
  const before = await target.screenshot({path: 'tests/screenshots/TC-13/before.png'});
  
  // Klik notasi yang akan dihapus pada kanvas
  await target.click({ position: {x: 180, y: 133} });

  // Tekan tombol Delete untuk menghapus notasi
  await page.keyboard.press('Delete');
  await page.waitForTimeout(500);

  // Verifikasi bahwa notasi terhapus dari kanvas
  const after = await target.screenshot({path: 'tests/screenshots/TC-13/after.png'});
