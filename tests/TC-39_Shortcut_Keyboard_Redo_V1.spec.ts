...
test('Menggunakan shortcut "Redo" yang valid melalui keyboard (Ctrl+Y)', async ({ page }) => {
  ...
  const before = await target.screenshot({path: 'tests/screenshots/TC-39/before.png'});

  // Melakukan Undo
  await page.locator('body').press('Control+Z');

  // Valid shortcut redo ctrl+Y menggunakan keyboard
  ...
  const after = await target.screenshot({path: 'tests/screenshots/TC-39/after.png'});
