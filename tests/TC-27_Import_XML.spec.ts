...
test('Import file dalam format XML melalui klik button', async ({ page }) => {
  ...
  const before = await target.screenshot({path: 'tests/screenshots/TC-27/before.png'});
  ...
  const after = await target.screenshot({path: 'tests/screenshots/TC-27/after.png'});
