...
test('Import file dalam format XML melalui drag&drop', async ({ page }) => {
  ...
  const before = await target.screenshot({path: 'tests/screenshots/TC-29/before.png'});
  ...
  const filePath = path.join(process.cwd(), 'downloads', 'diagram.xml');
  ...
  const after = await target.screenshot({ path: 'tests/screenshots/TC-29/after.png' });
